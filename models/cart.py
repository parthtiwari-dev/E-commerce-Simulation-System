...
class Cart:
    ...
    def view_items(self):
        ...
        for pid, (product,quantity) in self.items.items():
            ...
            total += subtotal
            print(f"{product.name:<20} {quantity:<5} ₹{product.price:<12.2f} ₹{subtotal:<10.2f}")
        print('-' * 55)
    ...
    def apply_coupon(self, coupon):
        ...
        if coupon not in self.applied_coupons:
            ...
            self.updated_at = datetime.now()
            print(f"Coupon {coupon.code if hasattr(coupon, 'code') else coupon} applied.")
        else:
            print(f"Coupon {coupon.code if hasattr(coupon, 'code') else coupon} is already applied.")
    ...
    def remove_coupon(self, coupon):
        ...
        if coupon in self.applied_coupons:
            ...
            self.updated_at = datetime.now()
            print(f"Coupon {coupon.code if hasattr(coupon, 'code') else coupon} removed.")
        else:
            print(f"Coupon {coupon.code if hasattr(coupon, 'code') else coupon} is not in the cart.")
    ...
    def get_cart_info(self):
        ...
        return {
            ...
            "products": products,
            "applied_coupons": [coupon.code if hasattr(coupon, "code") else str(coupon)
                                for coupon in self.applied_coupons],
            "subtotal": self.calculate_subtotal(),
